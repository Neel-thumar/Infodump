...
export default function Reader({ category, course, volume, rendered }) {
  ...
  const showToc = preferences.toc && tocItems.length > 1;
  ...
  return <main id="main" className={`reader-shell ${!preferences.sidebar ? 'hide-sidebar' : ''} ${!showToc ? 'hide-toc' : ''} ${!preferences.breadcrumbs ? 'hide-breadcrumbs' : ''} ${!preferences.title ? 'hide-title' : ''} ${!preferences.progress ? 'hide-progress' : ''} ${!preferences.footer ? 'hide-footer' : ''} ${!preferences.toolbar ? 'hide-toolbar' : ''} measure-${preferences.width}`} style={{ '--reader-font-size': `${preferences.fontSize}px` }}>
    ...
    <div className="reader-content">{volume && <ReaderControls volume={volume} chapter={chapter} rendered={rendered} preferences={preferences} setPreference={setPreference} reset={reset} menuRef={menuButton} drawerOpen={drawer} onMenu={() => { if (matchMedia('(max-width: 700px)').matches) setDrawer(!drawer); else setPreference('sidebar', !preferences.sidebar); }} />}<Breadcrumbs nodes={[category, course, ...(volume ? [volume] : [])]} />
      ...
      </div>{showToc && <aside className={`toc ${outlineOpen ? 'outline-expanded' : ''}`}><div className="toc-heading"><p className="eyebrow">ON THIS PAGE</p><button className="outline-expand" onClick={() => setOutlineOpen(!outlineOpen)} aria-expanded={outlineOpen} aria-label="Toggle section links">{outlineOpen ? '⌃' : '⌄'}</button><button onClick={() => setPreference('toc', false)} aria-label="Hide table of contents" title="Hide table of contents">×</button></div><nav aria-label="Table of contents">{tocItems.map(item => <a key={item.id} href={`#${item.id}`} aria-current={activeHeading === item.id ? 'location' : undefined} className={item.depth === 3 ? 'subheading' : ''}>{item.text}</a>)}</nav><a className="back-to-top" href="#main">↑ Back to top</a></aside>}</div>}
    </div>
  </main>;
}